...
const TiendasComponent: React.FC<Props> = ({ sedes, sedes_mayorista,videoPrincipal }) => {
  const [tipoTienda, setTipoTienda] = useState<"retail" | "mayorista">("retail");
  ...
  const tiendas = tipoTienda === "retail" ? agruparPorProvincia(sedes) : agruparPorProvincia(sedes_mayorista);
  ...
  return (
    <div className="p-4">
        ...
        <>
        {
          videoPrincipal.videoPrincipalDesk && (
            <video
              ...
              loop
              muted
              src={getSanityFileUrl(videoPrincipal.videoPrincipalDesk) || ""}
            ></video>
...

        {
          videoPrincipal.videoPrincipalMob && (
            <video
              ...
              loop
              muted 
              src={getSanityFileUrl(videoPrincipal.videoPrincipalMob) || ""}
            ></video>
          )
        }

        {
          videoPrincipal.videoPrincipalTablet && (
            <video
              ...
              loop
              muted
              src={getSanityFileUrl(videoPrincipal.videoPrincipalTablet) || ""}
            ></video>
          )
        }
        </>
      ...
      {tiendas
        .filter((provincia) => !provinciaSeleccionada || provincia.nombre === provinciaSeleccionada)
        .map((provincia) => (
      ...
      <ul className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 2xl:mx-auto 2xl:px-20 px-0 gap-y-20 xl:gap-y-0 lg:grid-cols-2 xl:grid-cols-3 2xl:gap-6  mt-6">
          ...
          el.dataSede.map((sede:Sede, i:number) => (
            <li key={index} className="p-4 border rounded shadow bg-gray-100 flex flex-col justify-between">
          ...
          <div className="p-4 flex items-center w-full justify-center">
            <div>
              ...
              <ul className="list-none">
                ...
                {sede?.horarios?.map((el:string, i:number) => (
                  <li
                    key={i}
